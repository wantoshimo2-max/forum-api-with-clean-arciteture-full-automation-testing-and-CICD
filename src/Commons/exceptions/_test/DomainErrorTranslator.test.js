...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    expect(DomainErrorTranslator.translate(new Error('NEW_THREAD.TITLE_LIMIT_CHAR')))
      .toStrictEqual(new InvariantError('tidak dapat membuat thread baru karena judul melebihi batas limit'));
    ...
    expect(DomainErrorTranslator.translate(new Error('DELETE_AUTH.NOT_CONTAIN_REFRESH_TOKEN')))
      .toStrictEqual(new InvariantError('harus mengirimkan token refresh'));
    expect(DomainErrorTranslator.translate(new Error('DELETE_AUTH.NOT_MEET_DATA_TYPE_SPECIFICATION')))
      .toStrictEqual(new InvariantError('refresh token harus string'));
    expect(DomainErrorTranslator.translate(new Error('REFRESH_AUTH.NOT_CONTAIN_REFRESH_TOKEN')))
      .toStrictEqual(new InvariantError('harus mengirimkan token refresh'));
